...
public class Perms {
    ...
    public static boolean isOwner(User user, TextChannel channel) {
        ...
        if (user.getId().equals(String.valueOf(STATICS.BOT_OWNER_ID)))
            return true;
        channel.sendMessage(MSGS.error().setDescription("Только владелец бота (" + user.getJDA().getUserById(STATICS.BOT_OWNER_ID).getAsMention() + ") может использовать эту команду.").build()).queue();
        return false;
